...
from data_preprocess import (build_datasets, DRUG_FEAT_DIM, PROTEIN_FEAT_DIM)
...
def main():
    ...
    if args.data_dir is None:
        args.data_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)),
    ...
    print(f"3DGLAD: GCN + CrossAttn + GAE | Seed: {SEED} | Device: {device}")

    print("Loading datasets...")
    train_ds, test_ds, dev_ds = build_datasets(args.data_dir)
    ...
    print(f"Total: {total_params:,} | Encoder: {encoder_params:,} | Decoder: {decoder_params:,}")

    best_ckpt_path = os.path.join(log_dir, 'best_model.pt')
    resume_ckpt_path = os.path.join(log_dir, 'resume_model.pt')
    ...
    print(f"\nPhase 2: DTI Fine-tuning (GCN + CrossAttn)")
    print("=" * 60)
    ...
    for epoch in range(start_epoch, 100):
        ...
        print(f"[GCN+CrossAttn] E{epoch+1}: Loss={total_loss/n:.4f} "
    ...
    print(f"\n[GCN+CrossAttn+GAE] Final Test Results (threshold={opt_thresh:.2f}):")
    print("-" * 40)
    ...
    print(f"Prediction stats: mean={test_preds.mean():.4f}, "
